...
from bootcamp_data.transforms import parse_datatime, add_time_parts, winsorize, add_outlier_flag
...
def main() -> None:
    ...
    orders_t = (
        orders
        .pipe(parse_datatime, col="created_at", utc=True)
        .pipe(add_time_parts, ts_col="created_at")
    )

    n_missing_ts = int(orders_t["created_at"].isna().sum())
    print(f"missing created_at after parse: {n_missing_ts} / {len(orders_t)}" )
